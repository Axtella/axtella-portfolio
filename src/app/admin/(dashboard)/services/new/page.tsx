...
import { InputField, TextareaField, ToggleField, FormSection } from "@/components/admin/form-field";
...
export default function NewServicePage() {
  ...
  const [formData, setFormData] = useState({
    title: "", slug: "", heroTitle: "", heroSubtitle: "", heroImage: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { toast("Service created", "success"); router.push("/admin/services"); }
    else { toast("Failed to create service", "error"); }
    setSaving(false);
  };
  ...
  return (
    <>
      ...
      <main className="p-6">
        <Link href="/admin/services" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-white mb-4 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-2 bg-[#F5A623] hover:bg-[#D4910A] text-black text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
              <Save size={16} />{saving ? "Saving..." : "Create Service"}
            </button>
          </div>
        </form>
      </main>
