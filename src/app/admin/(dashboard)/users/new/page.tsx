      ...
      <main className="p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
          <FormSection title="User Details">
            ...
            <div>
              <InputField label="Password" name="password" type="password" value={formData.password} onChange={(v) => update("password", v)} required hint="Minimum 8 characters" />
          ...
          <div className="flex justify-end gap-3">
            ...
            <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-2 bg-[#F5A623] hover:bg-[#D4910A] text-black text-sm font-medium rounded-lg transition-colors disabled:opacity-50"><Save size={16} />{saving ? "Saving..." : "Create User"}</button>
          </div>
        </form>
      </main>
    </>
