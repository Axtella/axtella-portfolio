...
export function ITHeroSection({ data }: { data?: ServiceHeroData | null }) {
  const lines = data?.heroTitle?.split("|") || [];
  const heroLine1 = lines[0] || "Powering Your";
  const heroLine2 = lines[1] || "Digital Future.";
  const heroSubtitle = data?.heroSubtitle || "Enterprise IT infrastructure, cloud solutions, cybersecurity, and digital transformation — designed for businesses across Saudi Arabia.";
